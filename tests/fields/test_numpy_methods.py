...
def test_concatenate(field):
    ...
    assert b.shape == new_shape
    assert np.array_equal(b[0:2,:], a1)
    assert np.array_equal(b[2:,:], a2)
    assert type(b) is field
    assert b.dtype == dtype
...
def test_resize(field):
    ...
    # c.resize(new_shape)
    # assert c.shape == new_shape
    # assert np.array_equal(c[0,:], a)
    # assert np.array_equal(c[1,:], 0)  # NOTE: This is different than np.resize()
    # assert type(c) is field
    # assert c.dtype == dtype
...
def test_roll(field):
    ...
    a = field.Random(shape, dtype=dtype)
    b = np.roll(a, 2)
    assert np.array_equal(b[0:2], a[-2:])
    assert np.array_equal(b[2:], a[0:-2])
    assert type(b) is field
