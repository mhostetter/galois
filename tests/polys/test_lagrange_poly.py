...
def test_binary_field():
    ...
    x = GF([0, 1])
    y = GF([0, 0])
    assert galois.lagrange_poly(x, y) == galois.Poly.String("0", field=GF)

    GF = galois.GF(2)
    x = GF([0, 1])
    y = GF([0, 1])
    assert galois.lagrange_poly(x, y) == galois.Poly.String("x", field=GF)

    GF = galois.GF(2)
    x = GF([0, 1])
    y = GF([1, 0])
    assert galois.lagrange_poly(x, y) == galois.Poly.String("x + 1", field=GF)

    x = GF([0, 1])
    y = GF([1, 1])
    assert galois.lagrange_poly(x, y) == galois.Poly.String("1", field=GF)
...
def test_prime_field():
    ...
    assert galois.lagrange_poly(x, y) == galois.Poly.String("207*x^9 + 222*x^8 + 227*x^7 + 88*x^6 + 6*x^5 + 52*x^4 + 82*x^3 + 198*x^2 + 27*x + 78", field=GF)
...
def test_binary_extension_field():
    ...
    assert galois.lagrange_poly(x, y) == galois.Poly.String("104*x^9 + 130*x^8 + 92*x^7 + 199*x^6 + 64*x^5 + 211*x^4 + 130*x^3 + 38*x^2 + 140*x + 114", field=GF)
...
def test_prime_extension_field():
    ...
    assert galois.lagrange_poly(x, y) == galois.Poly.String("42*x^9 + 200*x^8 + 109*x^7 + 82*x^6 + x^5 + 241*x^4 + 65*x^3 + 153*x^2 + 143*x + 88", field=GF)
