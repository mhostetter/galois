"""
A module that handles accessing the database of primitive polynomials.
"""
import os
import sqlite3

DATABASE = None  # Database singleton class
DATABASE_FILE = os.path.join(os.path.dirname(__file__), "primitive_polys.db")


class PrimitivePolyDatabase:
    """
    Class to interface with the primitive polynomials database.
    """
    ...
    def fetch(self, characteristic: int, degree: int):
        ...
        if result is None:
            raise LookupError(
                f"The primitive polynomials database does not contain an entry for GF({characteristic}^{degree}).\n\n"
